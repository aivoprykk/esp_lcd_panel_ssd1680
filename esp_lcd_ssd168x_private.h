...
#define SSD168X_RAM_SIZE                                                       \
	((SSD168X_SOURCE_SIZE * SSD168X_GATE_SIZE) << 3u) // 6512

#include "sdkconfig.h"
#if (defined(CONFIG_LOGGER_USE_GLOBAL_LOG_LEVEL) &&                            \
	 CONFIG_LOGGER_GLOBAL_LOG_LEVEL < CONFIG_SSD168X_LOG_LEVEL)
#define C_LOG_LEVEL CONFIG_LOGGER_GLOBAL_LOG_LEVEL
#else
#define C_LOG_LEVEL CONFIG_SSD168X_LOG_LEVEL
#endif
...
#define SSD168X_CHECK_IO_TX_PARAM(io, cmd, params, size, cmd_name)             \
	do {                                                                       \
		if (esp_lcd_panel_io_tx_param((io), (cmd), (params), (size)) !=        \
			ESP_OK) {                                                          \
			ELOG(TAG, "%s%s%s %s", drv_msg[0], drv_msg[1], (cmd_name),         \
				 drv_msg[2]);                                                  \
			return ESP_FAIL;                                                   \
		}                                                                      \
	} while (0)
...
#define SSD168X_CHECK_IO_TX_COLOR(io, cmd, data, size, cmd_name)               \
	do {                                                                       \
		if (esp_lcd_panel_io_tx_color((io), (cmd), (data), (size)) !=          \
			ESP_OK) {                                                          \
			ELOG(TAG, "%s%s %s", drv_msg[0], (cmd_name), drv_msg[2]);          \
			return ESP_FAIL;                                                   \
		}                                                                      \
	} while (0)
